...
| `render_report`           | 基于已有研究结果重新渲染报告 | `report_ready` -> `report_ready`      |
...
| 模块     | 文件                                           | 作用                               |
| ------ | -------------------------------------------- | -------------------------------- |
| 路由模块   | `app/routers/__init__.py`                    | 接收请求，创建任务记录，调用后台任务启动函数           |
| 后台任务模块 | `app/background/research_tasks.py`           | 封装 `asyncio.create_task`，执行长耗时流程 |
| 任务仓储模块 | `app/repository/research_task_repository.py` | 创建任务、查询任务、更新任务状态                 |
